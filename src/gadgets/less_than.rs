...
/// single expression.
pub fn expr_from_bytes<F: FieldExt>(bytes: &[Expression<F>]) -> Expression<F> {
...
pub fn pow_of_two<F: FieldExt>(by: usize) -> F {
    F::from(2).pow_vartime(&[by as u64, 0, 0, 0])
}
    ...
    fn assign(
        ...
        for (idx, diff_column) in config.diff.iter().enumerate() {
            region.assign_advice(
                ...
                *diff_column,
                offset,
                || diff_bytes.as_ref().map(|bytes| F::from(bytes.as_ref()[idx] as u64)),
            )?;
        }
    ...
    fn row_diff_is_lt() {
        ...
        impl<F: FieldExt> Circuit<F> for TestCircuit<F> {
            ...
            fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
                ...
                let config = Self::Config { q_enable, value, check, lt };
    ...
    fn column_diff_is_lt() {
        ...
        impl<F: FieldExt> Circuit<F> for TestCircuit<F> {
            ...
            fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
                ...
                let config = Self::Config { q_enable, value_a, value_b, check, lt };
        ...
        // ok
        try_test_circuit!(vec![(1, 2), (4, 4), (5, 5)], vec![true, false, false], Ok(()));
        try_test_circuit!(
            vec![(14124, 14124), (383168732, 383168731), (383168731, 383168732)],
