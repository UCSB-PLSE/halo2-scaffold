...
use halo2_scaffold::circuits::reduce_fraction::{generate_circuit, ReduceFractionCircuit};
use rand::rngs::OsRng;

#[derive(Parser)]
#[command(about = "I am a program and I work, just pass `-h`", long_about = None)]
struct Args {
    #[arg(short, help = "f.sign")]
    sign: bool,
    #[arg(short, help = "f.num")]
    num: u64,
    #[arg(short, help = "f.den")]
    den: u64,
    #[arg(short, help = "k number of rows")]
    k: u32,
}

fn run(sign: bool, num: u64, den: u64, k: u32) {
    use halo2_proofs::halo2curves::pasta::{Eq, Fp};
    let mcircuit: ReduceFractionCircuit<Fp> = generate_circuit(sign, num, den);
    let x = SimpleCircuitCost::<Eq, ReduceFractionCircuit<Fp>>::measure(k, &mcircuit);
    ...
    let circuit = generate_circuit(false, 90, 18);
